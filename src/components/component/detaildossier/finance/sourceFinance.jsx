            ...
            <Heading size='md' color='blue.500'>Règlements</Heading>
                        <UnorderedList>
                            {finance.reglements&&finance.reglements.map(item => 
                                    ...
                                    !isChecked?
                                    <ListItem key={item.id}>{item.no} -- {item.montant.toLocaleString('fr-CA', { style: 'currency', currency: 'CAD' })}</ListItem>:
                                    <ListItem key={item.id}><HStack ><Text>{item.no}-</Text>-<Input size='sm' type='currency' name={'reglements'} identifiant={item.no} value={item.montant} onChange={handleChange}></Input>
                        ...
                        {isChecked&&<HStack>                                    
                                    <MySelect label='Ajout règlement' name='reglements'>
                                        ...
                                        {reglements.map(item => <option value={item.id} key={item.id}>{item.numero}</option>)}
                                    </MySelect>
            ...
            <Heading size='md' color='blue.500'>Subventions</Heading>
                        <UnorderedList>
                                { finance.subventions&&finance.subventions.map(item =>                                    
                                    !isChecked?<ListItem key={item.id}>{item.nom} -- {item.montant.toLocaleString('fr-CA', { style: 'currency', currency: 'CAD' })}</ListItem>:
                                    <ListItem key={item.id}><HStack><Text>{item.nom} --</Text><Input size='sm' type='currency' name={'subventions'} identifiant={item.nom} value={item.montant} onChange={handleChange}></Input>
                        ...
                        {isChecked&&<HStack>
                                    <MySelect label='Ajout subvention' name='subventions'>
                                        ...
                                        {subventions.map(item => <option value={item.id} key={item.id}>{item.nomProg} -- {item.no_id}</option>)}
                                    </MySelect>
        ...
        >
            ...
            <Heading size='md' color='blue.500'>Fonds</Heading>
                        <UnorderedList>
                                ...
                                !isChecked?<ListItem key={item.id}>{item.nom} -- {item.montant.toLocaleString('fr-CA', { style: 'currency', currency: 'CAD' })}</ListItem>:
                                <ListItem key={item.id}><HStack><Text>{item.nom} --</Text><Input size='sm' type='currency' name={'fonds'} identifiant={item.nom} value={item.montant} onChange={handleChange}></Input>
                        ...
                        {isChecked&&<HStack>
                                    <MySelect label='Ajout fonds' name='fonds'>
                                        ...
                                        {fonds.map(item => <option value={item.id} key={item.id}>{item.nom}</option>)}
                                    </MySelect>
